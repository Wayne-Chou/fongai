/**
 * FongAI 文章資料庫
 * 所有的文章內容都管理在這裡
 */
const articlesData = [
 
  
  {
    id: "enterprise-2026-004",
    category: "enterprise",
    categoryName_zh: "合作案例",
    categoryName_en: "Case Studies",
    categoryName_ja: "導入事例",
    date: "2026-04-27",
    image: "../assets/images/cases/kaohsiung-fongai-bac.jpg",
  
    // ===== 中文 =====
    title_zh: "偏鄉健康行動力：高雄市政府運動發展局導入 FongAI 防跌鑑測",
    excerpt_zh:
      "高雄市運發局攜手全適能健康促進協會，將 FongAI 動態骨架防跌鑑測導入偏鄉社區，協助長者掌握行動力與跌倒風險...",
    content_zh: `
      <p>2024 年，高雄市政府運動發展局（運發局）於苓雅運動中心的樂齡運動活力站中，首次導入 FongAI 動態骨架防跌鑑測技術，協助學員了解自身行動能力與跌倒風險，為傳統運動課程加入更精準、科學化的評估工具。</p>
  
      <h3>科技導入運動課程</h3>
      <p>透過 AI 動態骨架辨識，學員只需完成簡單動作，即可獲得行動力評估與風險分析，使教練能更有效調整訓練內容，提升運動安全與效果。</p>
  
      <blockquote>「讓運動不只是活動，而是可量化的健康管理。」</blockquote>
  
      <h3>深入偏鄉，擴大健康影響力</h3>
      <p>2025 年，運發局透過台灣全適能健康促進協會，將 FongAI 防跌鑑測服務導入高雄偏鄉社區，結合既有運動課程，讓 108 位長者在課程中即時掌握自身行動能力。</p>
  
      <p>透過鑑測產出的個人化報告，長者能更具體了解自身健康狀況，進一步提升自我照護意識與課程參與動機。</p>
  
      <h3>建立長者與照護者的健康橋樑</h3>
      <p>結合鑑測與訓練課程，長者不僅能更安全地參與運動，也讓家人與照護者能即時掌握行動能力變化，及早提供必要支持與介入。</p>
  
      <h3>持續推動智慧健康照護</h3>
      <p>展望 2026 年，運發局將延續前兩年的成果，透過大型活動推廣 FongAI 防跌鑑測，讓更多高雄長者享有便利且可量化的健康管理方式，深化社區健康照護成效。</p>
  
      <img src="../assets/images/cases/kaohsiung-fongai.jpg" alt="高雄市政府運動發展局導入 FongAI 防跌鑑測">
  
      <h3>偏鄉 FongAI 防跌骨架鑑測</h3>
      <p>台灣全適能健康促進協會將 FongAI 技術帶入偏鄉，讓長者即使在資源較少的地區，也能享有 AI 科技的陪伴與照護。透過專業教練的肌力訓練介入，協助長者在熟悉的環境中安心樂活。</p>
    `,
  
    // ===== 英文 =====
    title_en: "Empowering Rural Health: FongAI Fall Risk Assessment Adopted by Kaohsiung Sports Development Bureau",
    excerpt_en:
      "The Kaohsiung Sports Development Bureau partners with the Taiwan Fitness Promotion Association to introduce FongAI AI-based fall risk assessment into rural communities...",
    content_en: `
      <p>In 2024, the Kaohsiung City Government Sports Development Bureau introduced FongAI’s AI-based fall risk assessment technology at the Lingya Sports Center, enhancing senior fitness programs with more precise and scientific evaluation tools.</p>
  
      <h3>Integrating Technology into Exercise Programs</h3>
      <p>Through AI-powered skeletal tracking, participants can complete simple movements to receive mobility assessments and fall risk analysis, enabling coaches to optimize training plans and improve both safety and effectiveness.</p>
  
      <blockquote>"Exercise is no longer just an activity, but measurable health management."</blockquote>
  
      <h3>Expanding Impact in Rural Communities</h3>
      <p>In 2025, the bureau collaborated with the Taiwan Fitness Promotion Association to bring FongAI services into rural areas of Kaohsiung. A total of 108 seniors were able to monitor their mobility in real time during exercise sessions.</p>
  
      <p>Personalized reports generated from the assessment provide clear and actionable insights, improving health awareness and encouraging active participation.</p>
  
      <h3>Connecting Seniors and Caregivers</h3>
      <p>By integrating assessment with training programs, seniors can participate more safely in physical activities, while families and caregivers gain better visibility into mobility conditions and can provide timely support.</p>
  
      <h3>Advancing Smart Community Healthcare</h3>
      <p>Looking ahead to 2026, the bureau plans to expand FongAI adoption through large-scale initiatives, bringing accessible and measurable health management solutions to more seniors across Kaohsiung.</p>
  
      <img src="../assets/images/cases/kaohsiung-fongai.jpg" alt="FongAI implementation in Kaohsiung">
  
      <h3>FongAI Fall Risk Assessment in Rural Areas</h3>
      <p>By introducing FongAI technology into underserved regions, seniors can benefit from AI-powered healthcare support even in remote communities, combined with professional coaching to promote safe and active living.</p>
    `,
  
    // ===== 日文 =====
    title_ja: "地方における健康支援：高雄市スポーツ発展局によるFongAI転倒リスク評価の導入",
    excerpt_ja:
      "高雄市スポーツ発展局は全適能健康促進協会と連携し、FongAIのAI転倒リスク評価を地方コミュニティへ導入...",
    content_ja: `
      <p>2024年、高雄市政府スポーツ発展局は苓雅スポーツセンターにおいて、FongAIのAI転倒リスク評価技術を初めて導入し、高齢者向け運動プログラムにより精密で科学的な評価を加えました。</p>
  
      <h3>運動プログラムへの技術導入</h3>
      <p>AIによる骨格認識を活用し、簡単な動作で身体能力や転倒リスクを評価できるため、指導者はより効果的で安全なトレーニングを提供できます。</p>
  
      <blockquote>「運動は単なる活動ではなく、測定可能な健康管理へ」</blockquote>
  
      <h3>地方への展開と影響拡大</h3>
      <p>2025年には全適能健康促進協会と連携し、高雄の地方地域へサービスを導入。108名の高齢者が運動中にリアルタイムで自身の身体能力を把握できるようになりました。</p>
  
      <p>個別レポートにより健康状態を可視化し、健康意識の向上とプログラム参加意欲の強化につながっています。</p>
  
      <h3>高齢者と介護者をつなぐ</h3>
      <p>評価とトレーニングを組み合わせることで、高齢者はより安全に運動に参加でき、家族や介護者も身体能力の変化を把握し、早期対応が可能になります。</p>
  
      <h3>スマートヘルスケアの推進</h3>
      <p>2026年には大規模な普及活動を通じて、より多くの高齢者に利便性の高い健康管理手法を提供し、地域医療の充実を目指します。</p>
  
      <img src="../assets/images/cases/kaohsiung-fongai.jpg" alt="高雄市でのFongAI導入">
  
      <h3>地方におけるFongAI転倒リスク評価</h3>
      <p>地方でもAI技術による健康支援が可能となり、高齢者が安心して生活できる環境づくりに貢献しています。</p>
    `,
  },
  {
    id: "enterprise-2026-005",
    category: "enterprise",
    categoryName_zh: "合作案例",
    categoryName_en: "Case Studies",
    categoryName_ja: "導入事例",
    date: "2026-04-27",
    image: "../assets/images/cases/fitness-fongai-bac.jpg",
  
    // ===== 中文 =====
    title_zh: "社區健康新模式：菲特邦健康管理顧問有限公司",
    excerpt_zh:
      "菲特邦導入 FongAI 動態骨架防跌鑑測於台北與台中社區課程，建立完整的長者健康管理模式...",
  
    content_zh: `
      <p>菲特邦健康管理顧問有限公司始終走在科技與服務融合的前端，持續將創新科技導入其運動服務體系中。2025 年，菲特邦全面導入 FongAI 動態骨架防跌鑑測於臺北社區運動課程，讓教練能即時掌握長者的行動能力，並依據數據調整課程內容與強度。</p>
  
      <h3>打造更精準的運動課程</h3>
      <p>透過即時鑑測數據，教練能更靈活安排課程，使訓練更加精準且安全，同時也讓長者在運動過程中更加安心，進一步強化教練與學員之間的信任與互動。</p>
  
      <h3>擴展至中部，深化社區健康</h3>
      <p>隨著臺北導入 FongAI 所展現的成效，菲特邦將服務拓展至臺中長者健康促進站，讓中部長者同樣能在課程中獲得即時身體評估，感受到社區照護的溫暖與專業支持。</p>
  
      <p>目前已於臺北 12 個、臺中 10 個社區據點完成導入，累計提供 352 人次鑑測服務，逐步建立完整的社區健康管理模式。</p>
  
      <h3>專屬健康報告強化使用者體驗</h3>
      <p>建豐與菲特邦共同設計專屬健康個人報告，讓學員在完成鑑測後能清楚了解自身狀態，進一步提升健康意識，同時也強化品牌專業形象。</p>
  
      <h3>打造以人為本的智慧照護</h3>
      <p>透過 FongAI 鑑測與社區課程的整合，雙方共同打造更完善的長者照護服務，讓每一位長者都能在社區中被看見、被關懷，讓健康促進的價值持續延伸。</p>
      <img src="../assets/images/cases/fitness-fongai.jpg" alt="菲特邦＆FongAI 聯名個人報告">
      <h3>菲特邦＆FongAI 聯名個人報告</h3>
      <p>報告正面以清晰易讀的方式呈現跌倒風險機率，背面則提供客製化運動建議與專屬課程體驗券，讓長者在理解自身健康狀態的同時，也提升課程參與與品牌黏著度。</p>
    `,
  
    // ===== 英文 =====
    title_en: "A New Community Health Model: Fitbon Health Management",
    excerpt_en:
      "Fitbon integrates FongAI fall-risk assessment into community programs in Taipei and Taichung, building a scalable senior health management model...",
  
    content_en: `
      <p>Fitbon Health Management has consistently been at the forefront of integrating technology into service innovation. In 2025, Fitbon fully implemented FongAI’s AI-based fall risk assessment across its community fitness programs in Taipei.</p>
  
      <h3>Enhancing Precision in Exercise Programs</h3>
      <p>With real-time assessment data, coaches can better understand seniors’ mobility and adjust training intensity accordingly, improving both safety and personalization while strengthening engagement between instructors and participants.</p>
  
      <h3>Expanding to Central Taiwan</h3>
      <p>Following successful outcomes in Taipei, Fitbon expanded the program into Taichung community health centers, allowing more seniors to benefit from real-time health insights and community-based care.</p>
  
      <p>The program has been deployed across 12 locations in Taipei and 10 in Taichung, completing a total of 352 assessments and establishing a scalable community health management model.</p>
  
      <h3>Personalized Health Reports</h3>
      <p>FongAI and Fitbon co-developed personalized health reports, enabling participants to clearly understand their mobility status and improving both engagement and brand trust.</p>
  
      <h3>Human-Centered Smart Healthcare</h3>
      <p>By combining AI assessment with community exercise programs, Fitbon and FongAI are creating a more comprehensive senior care ecosystem, ensuring every individual is supported and cared for.</p>
      <img src="../assets/images/cases/fitness-fongai.jpg" alt="菲特邦＆FongAI 聯名個人報告">
      <h3>Co-branded Health Report</h3>
      <p>The front side clearly presents fall risk probability, while the back includes customized exercise recommendations and exclusive program vouchers, enhancing both health awareness and user engagement.</p>
    `,
  
    // ===== 日文 =====
    title_ja: "地域健康の新モデル：フィットボン健康管理コンサルティング",
    excerpt_ja:
      "フィットボンはFongAIの転倒リスク評価を地域プログラムに導入し、高齢者の健康管理モデルを構築...",
  
    content_ja: `
      <p>フィットボン健康管理コンサルティングは、常にテクノロジーとサービスを融合した革新に取り組んでいます。2025年、台北の地域運動プログラムにFongAIのAI転倒リスク評価を全面導入しました。</p>
  
      <h3>より精密な運動プログラムの実現</h3>
      <p>リアルタイムデータにより、高齢者の身体能力を把握し、トレーニング内容を最適化。安全性と効果を高めながら、指導者と参加者の関係性も強化されました。</p>
  
      <h3>中部地域への展開</h3>
      <p>台北での成功を受け、台中の健康促進拠点にも展開。より多くの高齢者が地域での健康支援を受けられるようになりました。</p>
  
      <p>現在、台北12拠点・台中10拠点に導入され、累計352件の評価を実施し、地域健康管理モデルを構築しています。</p>
  
      <h3>パーソナライズされた健康レポート</h3>
      <p>フィットボンとFongAIは共同で個別健康レポートを開発し、利用者が自身の状態を明確に理解できるよう支援しています。</p>
  
      <h3>人中心のスマートケア</h3>
      <p>AI評価と地域プログラムの統合により、高齢者一人ひとりに寄り添った包括的なケアを実現しています。</p>
      <img src="../assets/images/cases/fitness-fongai.jpg" alt="菲特邦＆FongAI 聯名個人報告">
      <h3>共同ブランド健康レポート</h3>
      <p>レポートの表面には転倒リスクが分かりやすく表示され、裏面には個別運動提案と体験クーポンが含まれています。</p>
    `,
  },
  {
    id: "enterprise-2026-006",
    category: "enterprise",
    categoryName_zh: "合作案例",
    categoryName_en: "Case Studies",
    categoryName_ja: "導入事例",
    date: "2026-04-27",
    image: "../assets/images/cases/home-care-fongai.jpg",
  
    // ===== 中文 =====
    title_zh: "把關懷送到家：全成獨居長者到府 FongAI 鑑測服務",
    excerpt_zh:
      "全成社會福利基金會導入 FongAI 到府鑑測服務，將 AI 科技帶入台中獨居長者日常照護，提升跌倒風險預警與健康管理能力...",
  
    content_zh: `
      <p>全成社會福利基金會自 2025 年起積極推動「科技照顧」計畫，將 AI 技術導入臺中市獨居長者的到府關懷服務。每一次進入家門，不只是陪伴與問候，更增加了一層對居家安全與跌倒風險的提前預警。</p>
  
      <h3>讓科技真正走進日常生活</h3>
      <p>透過手機即可完成的 FongAI 動態骨架防跌鑑測，全成讓「關懷」從感受轉化為可量化的健康數據，並已於龍井、大里等 9 個行政區逐步推廣，讓科技真正融入長者生活。</p>
  
      <h3>2 分鐘完成行動力評估</h3>
      <p>透過 SPPB 簡短下肢功能評估，長者只需約 2 分鐘即可完成測試。系統自動分析重心偏移、姿勢傾斜與步態穩定性，並依國際標準進行行動力分級，讓長者與照護人員都能清楚掌握身體狀況。</p>
  
      <h3>從風險辨識到實際介入</h3>
      <p>當鑑測結果顯示高風險或衰退跡象，全成立即協助轉介至延緩失能課程或健走活動，並持續追蹤與調整訓練方案，讓健康管理真正落地。</p>
  
      <p>建豐也定期提供群體數據報告，協助基金會掌握整體健康趨勢，並透過個案研討機制，提前辨識身體狀況明顯衰弱的長者，提升照護精準度。</p>
      <img src="../assets/images/cases/home-care-fongai2.jpg" alt="全成">
      <h3>科技與關懷的結合獲得肯定</h3>
      <p>在 2025 年衛福部策展中，全成社會福利基金會榮獲「最佳創新獎」，由副署長親自頒發。這不僅代表 AI 技術的價值，更展現全成將科技融入長者日常照護的長期努力。</p>
  
      <p>這項成果證明，科技不只是工具，而是能真正走進生活、被信任並持續帶來安心的陪伴。</p>
    `,
  
    // ===== 英文 =====
    title_en: "Bringing Care Home: FongAI Home Assessment Service for Seniors Living Alone",
    excerpt_en:
      "The Chuan Cheng Social Welfare Foundation integrates FongAI home-based assessments into elderly care services in Taichung, enhancing fall risk prevention and health monitoring...",
  
    content_en: `
      <p>Since 2025, the Chuan Cheng Social Welfare Foundation has actively promoted a “Tech-Enabled Care” initiative, introducing AI technology into home visit services for seniors living alone in Taichung.</p>
  
      <h3>Technology Integrated into Daily Life</h3>
      <p>With FongAI’s mobile-based fall risk assessment, care services are transformed into measurable and trackable health insights. The program has expanded across 9 districts, bringing AI-powered care into everyday senior life.</p>
  
      <h3>2-Minute Mobility Assessment</h3>
      <p>Using the SPPB (Short Physical Performance Battery), seniors can complete a mobility assessment in just two minutes. The system analyzes balance, posture deviation, and gait stability, providing standardized mobility classifications.</p>
  
      <h3>From Detection to Intervention</h3>
      <p>When high-risk conditions are detected, seniors are immediately referred to rehabilitation programs or walking exercises, with continuous monitoring and adjustment to ensure effective outcomes.</p>
  
      <p>FongAI also provides aggregated reports to help the foundation track overall health trends and identify individuals requiring early intervention.</p>
      <img src="../assets/images/cases/home-care-fongai2.jpg" alt="全成">
      <h3>Recognition of Innovation</h3>
      <p>In 2025, the foundation received the “Best Innovation Award” from Taiwan’s Ministry of Health and Welfare, recognizing its success in integrating AI into real-world elderly care.</p>
  
      <p>This achievement demonstrates that technology is not just a tool, but a trusted companion that enhances safety and quality of life.</p>
    `,
  
    // ===== 日文 =====
    title_ja: "ケアを自宅へ：単身高齢者向けFongAI訪問評価サービス",
    excerpt_ja:
      "全成社会福祉基金会はFongAIを導入し、台中の単身高齢者向け在宅ケアにAI評価を組み込みました...",
  
    content_ja: `
      <p>全成社会福祉基金会は2025年より「テクノロジーケア」プロジェクトを推進し、台中市の単身高齢者向け訪問サービスにAI技術を導入しました。</p>
  
      <h3>日常生活に溶け込むテクノロジー</h3>
      <p>FongAIのモバイル評価により、ケアは定量化された健康データへと進化し、9つの行政区に展開されています。</p>
  
      <h3>2分で完了する身体機能評価</h3>
      <p>SPPBを活用し、短時間で身体能力を評価。バランス、姿勢、歩行の安定性を分析し、標準化された指標で可視化します。</p>
  
      <h3>リスク検知から介入へ</h3>
      <p>高リスクが検出された場合、リハビリや運動プログラムへ即時連携し、継続的なフォローアップを行います。</p>
  
      <p>また、集計レポートを活用し、個別ケースの早期発見と対応を実現しています。</p>
      <img src="../assets/images/cases/home-care-fongai2.jpg" alt="全成">
      <h3>革新への評価</h3>
      <p>2025年、同基金会は台湾衛生福利部より「最優秀イノベーション賞」を受賞しました。</p>
  
      <p>この取り組みは、テクノロジーが単なるツールではなく、安心を支える存在であることを示しています。</p>
    `,
  },
  {
    id: "enterprise-2026-007",
    category: "enterprise",
    categoryName_zh: "合作案例",
    categoryName_en: "Case Studies",
    categoryName_ja: "導入事例",
    date: "2026-04-27",
    image: "../assets/images/cases/taipei-wanhua-fongai.jpg",
    
    // ===== 中文 =====
    title_zh: "用科技守護萬華：台北市立心基金會導入 FongAI 防跌鑑測",
    excerpt_zh:
      "立心基金會於萬華龍山據點導入 FongAI 防跌鑑測，累積超過 600 人次數據，建立可量化的長者行動力管理模式...",
  
    content_zh: `
      <p>自 2023 年起，財團法人台北市立心基金會於萬華龍山老人服務暨日間照顧中心率先導入 FongAI AI 防跌骨架鑑測服務，成為台北市首批以科技守護長者行動力的示範場域。</p>
  
      <p>截至目前已累積超過 600 人次鑑測，協助萬華區一般長者與獨居長者建立可量化、可追蹤的行動力紀錄。</p>
  
      <h3>從鑑測走向真正的改變</h3>
      <p>立心基金會的核心價值不僅在於「測得出來」，更在於「做得到改變」。中心每年依據鑑測結果，主動安排高風險長者參與復能課程與肌力訓練，實際改善下肢肌力、行走穩定度與居家安全。</p>
  
      <p>許多原本因風險而減少活動的長者，也在課程中逐步恢復自信與外出意願。</p>
  
      <h3>將關懷延伸至獨居長者</h3>
      <p>在獨居長者服務上，立心基金會更進一步將照護從物資與陪伴，擴展到「行動力照護」與「居家安全預防」，讓科技成為可被感受到的實質關懷。</p>
  
      <h3>持續擴展服務影響力</h3>
      <p>自 2026 年起，立心基金會將此成功模式推展至大同老人服務暨日間照顧中心，進一步擴大健康促進服務規模。</p>
  
      <p>透過 AI 科技結合專業照顧，立心基金會正以實際行動證明：長者的行動能力不僅可以被量化，更可以被持續改善。</p>
      <div class="case-images">
        <div class="case-image">
          <img src="../assets/images/cases/taipei-wanhua-fongai2.jpg" alt="台北市立心基金會">
        </div>
        <div class="case-image">
          <img src="../assets/images/cases/taipei-wanhua-fongai3.png" alt="台北市立心基金會">
        </div>
      </div>
      <h3>復能課程</h3>
      <p>透過專業肌力訓練課程，協助長者提升平衡能力與肌耐力，降低跌倒風險，讓日常生活更加安全。</p>
      <div class="case-images">
        <div class="case-image">
          <img src="../assets/images/cases/taipei-wanhua-fongai.jpg" alt="台北市立心基金會">
        </div>
        <div class="case-image">
          <img src="../assets/images/cases/taipei-wanhua-fongai4.png" alt="台北市立心基金會">
        </div>
      </div>
      <h3>FongAI 防跌骨架鑑測</h3>
      <p>透過課程前後的 FongAI 鑑測，量化呈現身體能力的進步幅度，讓長者能清楚看見自身成長，進一步提升持續參與的動機與信心。</p>
    `,
  
    // ===== 英文 =====
    title_en: "Protecting Wanhua with Technology: FongAI Adoption by Lixin Foundation",
    excerpt_en:
      "The Lixin Foundation integrates FongAI fall-risk assessment in Wanhua, exceeding 600 assessments and building a measurable mobility care model...",
  
    content_en: `
      <p>Since 2023, the Lixin Foundation has introduced FongAI AI-based fall risk assessment at the Wanhua Longshan Senior Service Center, becoming one of Taipei’s first pilot sites using technology to protect senior mobility.</p>
  
      <p>To date, over 600 assessments have been conducted, helping both community-dwelling and elderly individuals living alone establish measurable and trackable mobility records.</p>
  
      <h3>From Assessment to Real Change</h3>
      <p>The foundation’s focus goes beyond measurement—it emphasizes meaningful improvement. Based on assessment results, high-risk seniors are enrolled in rehabilitation and strength training programs to improve lower body strength, walking stability, and home safety.</p>
  
      <p>Many seniors who were once hesitant to move have regained confidence and motivation to engage in daily activities.</p>
  
      <h3>Extending Care to Seniors Living Alone</h3>
      <p>Lixin expands care beyond companionship and supplies, incorporating mobility care and home safety prevention into services for seniors living alone, making technology a tangible form of support.</p>
      <div class="case-images">
        <div class="case-image">
          <img src="../assets/images/cases/taipei-wanhua-fongai2.jpg" alt="台北市立心基金會">
        </div>
        <div class="case-image">
          <img src="../assets/images/cases/taipei-wanhua-fongai3.png" alt="台北市立心基金會">
        </div>
      </div>
      <h3>Scaling Impact</h3>
      <p>Starting in 2026, this successful model will be extended to the Datong Senior Service Center, expanding its reach and impact.</p>
  
      <p>By integrating AI technology with professional care, Lixin demonstrates that mobility can be measured, monitored, and improved over time.</p>
  
      <h3>Rehabilitation Programs</h3>
      <p>Through structured strength training, seniors improve balance and endurance, reducing fall risks and enhancing daily safety.</p>
      <div class="case-images">
        <div class="case-image">
          <img src="../assets/images/cases/taipei-wanhua-fongai.jpg" alt="台北市立心基金會">
        </div>
        <div class="case-image">
          <img src="../assets/images/cases/taipei-wanhua-fongai4.png" alt="台北市立心基金會">
        </div>
      </div>
      <h3>FongAI Fall Risk Assessment</h3>
      <p>Pre- and post-program assessments quantify improvements, allowing seniors to clearly see their progress and boosting their confidence in continued participation.</p>
    `,
  
    // ===== 日文 =====
    title_ja: "万華をテクノロジーで守る：立心基金会によるFongAI導入",
    excerpt_ja:
      "立心基金会は万華地区でFongAI評価を導入し、600件以上のデータを蓄積しながら高齢者ケアモデルを構築...",
  
    content_ja: `
      <p>2023年より、立心基金会は台北市万華龍山高齢者サービスセンターにてFongAIのAI転倒リスク評価を導入し、高齢者の行動能力を守る先進的な取り組みを開始しました。</p>
  
      <p>これまでに600件以上の評価を実施し、高齢者の身体機能を定量的に記録・管理しています。</p>
  
      <h3>評価から改善へ</h3>
      <p>評価結果を基に、高リスクの高齢者に対してリハビリや筋力トレーニングを提供し、歩行安定性や生活安全性を向上させています。</p>
  
      <p>活動に消極的だった高齢者も、自信を取り戻し、日常生活への参加意欲が高まっています。</p>
  
      <h3>独居高齢者への支援拡大</h3>
      <p>支援は物資提供や訪問に留まらず、行動能力の維持と安全予防へと拡張され、テクノロジーが実感できる支援へと進化しています。</p>
  
      <h3>サービスの拡張</h3>
      <p>2026年には大同地区へ展開し、より多くの高齢者へサービスを提供する予定です。</p>
  
      <p>AIと専門ケアの融合により、行動能力は「見える化」され、継続的な改善が可能となっています。</p>
      
      <h3>リハビリプログラム</h3>
      <p>筋力トレーニングにより、バランスと持久力を向上させ、転倒リスクを低減します。</p>
      <div class="case-images">
        <div class="case-image">
          <img src="../assets/images/cases/taipei-wanhua-fongai.jpg" alt="台北市立心基金會">
        </div>
        <div class="case-image">
          <img src="../assets/images/cases/taipei-wanhua-fongai4.png" alt="台北市立心基金會">
        </div>
      </div>
      <h3>FongAI転倒リスク評価</h3>
      <p>前後比較により改善度を可視化し、高齢者の継続参加意欲を高めます。</p>
    `,
  },
  {
    id: "enterprise-2026-008",
    category: "enterprise",
    categoryName_zh: "合作案例",
    categoryName_en: "Case Studies",
    categoryName_ja: "導入事例",
    date: "2026-04-27",
    image: "../assets/images/cases/xinyi-fongai.jpg",
  
    // ===== 中文 =====
    title_zh: "AI 防跌鑑測打造高齡友善社區｜臺北市信義區健康服務中心",
    excerpt_zh:
      "建豐健康科技與信義區健康服務中心合作，導入 FongAI AI 防跌鑑測於公共衛生流程，建立可追蹤的長者健康管理模式...",
  
    content_zh: `
      <h3>AI 防跌鑑測導入公共衛生第一線</h3>
      <p>臺北市信義區健康服務中心長期推動健康城市、高齡友善與社區健康促進服務，面對高齡人口增加與長者跌倒風險上升，如何在社區第一線及早辨識長者行動力退化，成為公共衛生服務的重要課題。</p>
  
      <p>建豐健康科技與信義區健康服務中心合作，將 FongAI AI 動態骨架防跌鑑測導入社區服務流程，協助長者透過簡易、快速、免穿戴的方式完成行動力評估。透過 AI 影像分析，系統可協助判讀長者平衡、起坐與行走表現，並提供跌倒風險評估結果，讓健康服務中心能更有效掌握社區長者的行動力狀態。</p>
  
      <h3>從檢測到介入：建立可追蹤的長者健康促進流程</h3>
      <p>這項合作的重點不只是「完成檢測」，而是將 AI 鑑測結果進一步串接至後續健康促進服務。根據臺北市政府衛生局公開資訊，信義區健康服務中心結合 AI 動態防跌骨架鑑測 APP 與國民健康署 ICOPE 長者健康整合式功能評估工具，建立長者跌倒風險安全防護網，並已運用 AI 鑑測完成 208 位長者的跌倒風險檢測。針對高風險個案，信義區也安排延緩失能班、轉介據點、醫院與藥局等資源，經 3 個月運動衛教或課程介入後，跌倒風險機率由 18% 降至 15%。</p>
  
      <p>對地方政府與健康服務中心而言，這代表 FongAI 不只是單點式科技工具，而是能被納入公共衛生流程的前端篩檢入口。透過「鑑測 → 風險辨識 → 衛教建議 → 課程介入 → 成效追蹤」的流程，長者的健康變化能被數據化呈現，也讓公部門服務成果更容易被管理、追蹤與對外說明。</p>
  
      <h3>政府記者會公開展示：讓 AI 防跌服務被更多單位看見</h3>
      <p>在臺北市政府衛生局相關記者會中，建豐健康科技創辦人林凱馳也到場參與說明與 Demo，現場展示 FongAI 如何以手機或平板進行免穿戴檢測。公開報導指出，長者依指示完成五下起坐、平衡與行走三項動作後，系統即可透過 AI 影像分析技術評估跌倒風險，並依據 Vivifrail 標準進行分級。</p>
  
      <p>這場公開展示的意義，不只是一次媒體曝光，而是讓地方政府、健康服務中心與公共衛生單位能具體理解：AI 防跌鑑測可以如何落地到社區場域、如何被長者接受、如何成為政策推動中的數據化工具。對建豐而言，這也強化了 FongAI 在 B2G 場域中的可導入性與可複製性。</p>
  
      <h3>建豐 × 信義區健康服務中心的核心成效</h3>
      <p>1. 將 AI 防跌鑑測導入政府公共衛生服務</p>
      <p>FongAI 協助健康服務中心在社區中快速執行長者行動力評估，使跌倒風險辨識不再只依賴主觀詢問，而能透過數據輔助判斷。</p>

      <p>2. 建立長者行動力資料與追蹤基礎</p>
      <p>透過前後測與量化結果，健康服務中心能掌握長者行動力變化，並作為課程安排、衛教建議與後續追蹤的依據。</p>

      <p>3. 串接延緩失能與社區資源轉介</p>
      <p>鑑測結果可協助辨識高風險長者，並銜接延緩失能課程、社區據點、醫療院所或藥局等資源，提高公共衛生服務的連續性。</p>

      <p>4. 形成可複製的 B2G 合作模型</p>
      <p>此案例展現 FongAI 可導入地方政府、健康服務中心與高齡友善城市計畫，協助公部門以更具效率與數據基礎的方式推動長者防跌服務。</p>

      <h3>可複製合作模式｜適合地方政府與健康服務中心導入</h3>
      <p>建豐可協助地方政府或健康服務中心建立以下服務流程：</p>

      <p>導入 FongAI AI 動態骨架防跌鑑測</p>
      <p>於健康服務中心、社區據點或活動現場設置檢測流程</p>
      <p>完成長者行動力與跌倒風險評估</p>
      <p>依據結果提供衛教、課程建議或資源轉介</p>
      <p>進行前後測追蹤，建立量化成果</p>
      <p>彙整場域資料，作為政策成果、專案報告與擴點依據</p>

      <h3>結語｜從單次檢測走向政府級長者行動力管理</h3>
      <p>建豐健康科技與臺北市信義區健康服務中心的合作，展現了 AI 科技導入公共衛生場域的實際價值。透過 FongAI 動態骨架防跌鑑測，長者能更早了解自身行動力與跌倒風險，健康服務中心也能以數據為基礎，安排後續介入、轉介與成效追蹤。</p>

      <p>這項合作不只是一次健康活動，而是一套可延伸至其他行政區、縣市政府與高齡友善城市計畫的 B2G 服務模式。對正在尋找長者防跌、健康促進與延緩失能解決方案的公部門而言，信義區經驗提供了一個可落地、可追蹤、也可擴散的成功案例。</p>
      <img style="max-width:500px" src="../assets/images/cases/xinyi-fongai2.jpg" alt="臺北市信義區健康服務中心">
      <p>信義區寒冬送暖活動FongAI動態骨架行動力鑑測</p>
    `,
  
    // ===== 英文 =====
    title_en: "AI Fall Risk Assessment for Age-Friendly Communities | Taipei Xinyi Health Center",
    excerpt_en:
      "FongAI collaborates with Taipei Xinyi Health Center to integrate AI fall risk assessment into public health services and build a trackable senior care model...",
  
      content_en: `
      <h3>AI Fall Risk Assessment Integrated into Public Health Frontlines</h3>
      <p>The Taipei Xinyi District Health Center has long been committed to promoting healthy cities, age-friendly environments, and community health services. With the increasing aging population and rising fall risks among seniors, how to identify mobility decline early at the community level has become a critical issue in public health services.</p>
      
      <p>FongAI collaborated with the Xinyi Health Center to integrate AI-based skeletal fall risk assessment into community service workflows, enabling seniors to complete mobility evaluations through a simple, fast, and non-wearable process. Through AI image analysis, the system can assess balance, sit-to-stand, and walking performance, and provide fall risk evaluation results, allowing the health center to better understand the mobility conditions of seniors in the community.</p>
      
      <h3>From Assessment to Intervention: Building a Trackable Health Promotion Workflow</h3>
      <p>This collaboration focuses not only on “completing assessments,” but also on connecting AI assessment results to follow-up health promotion services. According to public information from the Taipei City Department of Health, the Xinyi Health Center combined an AI fall risk assessment app with the ICOPE integrated care tool to establish a fall prevention network. A total of 208 seniors have completed fall risk assessments. For high-risk individuals, the center arranged disability prevention programs and referrals to community centers, hospitals, and pharmacies. After three months of intervention, fall risk decreased from 18% to 15%.</p>
      
      <p>For local governments and health centers, this demonstrates that FongAI is not just a standalone technology, but a front-end screening tool that can be integrated into public health workflows. Through the process of “assessment → risk identification → health education → intervention → outcome tracking,” changes in senior health can be quantified, and public service outcomes can be more easily managed, tracked, and communicated.</p>
      
      <h3>Public Demonstration: Expanding the Visibility of AI Fall Prevention Services</h3>
      <p>At a press conference held by the Taipei City Department of Health, FongAI founder Lin Kai-Chih also participated in demonstrations, showcasing how the system can perform non-wearable assessments using mobile phones or tablets. Reports indicated that seniors completed five sit-to-stand movements, balance, and walking tests, and the system analyzed fall risk using AI image analysis and classified results according to Vivifrail standards.</p>
      
      <p>The significance of this public demonstration lies not only in media exposure, but also in helping government agencies, health centers, and public health organizations understand how AI fall risk assessment can be implemented in community settings, accepted by seniors, and used as a data-driven tool for policy development. For FongAI, this further strengthens its applicability and scalability in B2G scenarios.</p>
      
      <h3>Key Outcomes of FongAI × Xinyi Health Center</h3>
      <p>1. Integration of AI fall risk assessment into public health services</p>
      <p>FongAI enables health centers to quickly conduct mobility assessments for seniors in the community, ensuring that fall risk identification is no longer solely based on subjective judgment, but supported by data-driven insights.</p>

      <p>2. Establishment of senior mobility data and tracking foundation</p>
      <p>Through pre- and post-assessments and quantitative results, health centers can monitor changes in senior mobility and use the data for course planning, health education, and follow-up tracking.</p>

      <p>3. Integration with disability prevention programs and community resources</p>
      <p>Assessment results help identify high-risk individuals and connect them with prevention programs, community centers, hospitals, and pharmacies, enhancing the continuity of public health services.</p>

      <p>4. Formation of a scalable B2G collaboration model</p>
      <p>This case demonstrates that FongAI can be integrated into local governments, health centers, and age-friendly city initiatives, enabling the public sector to promote fall prevention services more efficiently with a data-driven approach.</p>

      <h3>Scalable Collaboration Model｜Suitable for Local Governments and Health Centers</h3>
      <p>FongAI can support local governments and health centers in building the following workflow:</p>

      <p>Implement FongAI AI-based skeletal fall risk assessment</p>
      <p>Set up assessment processes in health centers, community locations, or event sites</p>
      <p>Conduct mobility and fall risk evaluations for seniors</p>
      <p>Provide health education, course recommendations, or resource referrals based on results</p>
      <p>Perform pre- and post-assessment tracking to establish measurable outcomes</p>
      <p>Aggregate site data for policy outcomes, project reports, and expansion planning</p>

      <h3>Conclusion｜From Single Assessment to Government-Level Mobility Management</h3>
      <p>The collaboration between FongAI and the Taipei Xinyi District Health Center demonstrates the real value of integrating AI technology into public health settings. Through FongAI’s skeletal fall risk assessment, seniors can better understand their mobility and fall risks, while health centers can use data to guide intervention, referral, and outcome tracking.</p>

      <p>This collaboration is not just a one-time health activity, but a scalable B2G service model that can be extended to other districts, cities, and age-friendly initiatives. For public sector organizations seeking fall prevention, health promotion, and disability prevention solutions, the Xinyi model provides a practical, trackable, and scalable success case.</p>
      <img style="max-width:500px" src="../assets/images/cases/xinyi-fongai2.jpg" alt="臺北市信義區健康服務中心">
      <p>Xinyi District Winter Care Event – FongAI Mobility Assessment</p>
      `,
  
    // ===== 日文 =====
    title_ja: "AI転倒リスク評価で高齢者に優しい地域づくり｜台北市信義区健康センター",
    excerpt_ja:
      "FongAIは信義区健康センターと連携し、AI転倒リスク評価を公共衛生に導入...",
  
      content_ja: `
      <h3>公共衛生の最前線に導入されたAI転倒リスク評価</h3>
      <p>台北市信義区健康センターは、健康都市、高齢者に優しい環境、地域健康促進を長年推進してきました。高齢化の進行と転倒リスクの増加に伴い、地域レベルで高齢者の身体機能低下を早期に把握することが公共衛生の重要な課題となっています。</p>
      
      <p>FongAIは信義区健康センターと連携し、AI動態骨格による転倒リスク評価を地域サービスに導入しました。高齢者は簡単・迅速・非装着型で評価を受けることができ、AI画像解析によりバランス、立ち上がり、歩行動作を分析し、転倒リスクを可視化します。これにより、地域全体の行動能力状況を把握することが可能になります。</p>
      
      <h3>評価から介入へ：追跡可能な健康促進プロセスの構築</h3>
      <p>この取り組みは単なる「評価の実施」にとどまらず、AI評価結果をその後の健康促進サービスへと連携させる点にあります。台北市衛生局の公開情報によると、信義区健康センターはAI転倒リスク評価アプリとICOPE統合評価ツールを組み合わせ、転倒リスク防護ネットワークを構築し、208名の高齢者の評価を実施しました。高リスク対象者には運動プログラムや地域・医療資源への紹介を行い、3ヶ月後には転倒リスクが18%から15%へ低下しました。</p>
      
      <p>地方政府や健康センターにとって、これはFongAIが単なる単体技術ではなく、公共衛生プロセスに組み込まれるフロントエンドのスクリーニングツールであることを意味します。「評価 → リスク判定 → 健康指導 → 介入 → 成果追跡」の流れにより、高齢者の健康状態はデータとして可視化され、管理・追跡・説明が容易になります。</p>
      
      <h3>記者会見での公開展示：AI転倒予防サービスの認知拡大</h3>
      <p>台北市衛生局の記者会見では、FongAI創業者 林凱馳氏がデモを行い、スマートフォンやタブレットによる非装着型評価を紹介しました。報道によると、高齢者は椅子立ち上がり5回、バランス、歩行の3動作を行うことで、AI画像解析により転倒リスクが評価され、Vivifrail基準に基づき分類されます。</p>
      
      <p>この公開展示は単なるメディア露出ではなく、政府機関や健康センター、公共衛生機関に対し、AI転倒リスク評価がどのように地域に導入され、どのように受け入れられ、政策推進のデータツールとして活用できるかを具体的に示す機会となりました。また、FongAIのB2G分野における導入可能性と拡張性を強化するものでもあります。</p>
      
      <h3>建豐 × 信義区健康センターの主な成果</h3>
      <p>1. AI転倒リスク評価の公共衛生サービスへの導入</p>
      <p>FongAIは地域において高齢者の行動能力評価を迅速に実施できるよう支援し、転倒リスクの判定を主観的判断だけに依存せず、データに基づいた判断を可能にします。</p>

      <p>2. 高齢者の行動能力データと追跡基盤の構築</p>
      <p>前後評価と定量データにより、高齢者の身体能力の変化を把握し、プログラム設計や健康指導、継続的なフォローに活用できます。</p>

      <p>3. 介護予防プログラムおよび地域資源との連携</p>
      <p>評価結果により高リスク対象者を特定し、介護予防プログラムや地域拠点、医療機関、薬局などと連携することで、公共衛生サービスの継続性を向上させます。</p>

      <p>4. 再現可能なB2G協力モデルの構築</p>
      <p>本事例は、FongAIが地方政府や健康センター、高齢者に優しい都市政策へ導入可能であり、データに基づいた効率的な転倒予防サービスを実現できることを示しています。</p>

      <h3>展開可能な協力モデル｜地方政府・健康センター向け</h3>
      <p>建豐は以下のサービスフロー構築を支援します：</p>

      <p>FongAI AI転倒リスク評価の導入</p>
      <p>健康センターや地域拠点、イベント会場での評価プロセス構築</p>
      <p>高齢者の行動能力および転倒リスク評価の実施</p>
      <p>結果に基づく健康指導・プログラム提案・資源紹介</p>
      <p>前後比較による成果追跡</p>
      <p>データ集約による政策・レポート・拡張活用</p>

      <h3>結論｜単発評価から政府レベルの健康管理へ</h3>
      <p>建豐と台北市信義区健康センターの連携は、AI技術を公共衛生分野に導入する実質的な価値を示しています。FongAIにより、高齢者は自身の身体能力と転倒リスクをより早期に把握でき、健康センターはデータに基づいた介入・紹介・追跡が可能になります。</p>

      <p>この取り組みは単なる一度の健康イベントではなく、他地域や自治体へ展開可能なB2Gモデルです。転倒予防や健康促進、介護予防を検討する公共機関にとって、信義区の事例は実践的かつ拡張可能な成功モデルとなります。</p>
      <img style="max-width:500px" src="../assets/images/cases/xinyi-fongai2.jpg" alt="臺北市信義區健康服務中心">
      <p>信義区 冬季支援イベント FongAI行動能力評価</p>
      `,
  }
];
